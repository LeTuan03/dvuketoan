...
import { Newspaper, ChevronRight, Building2, MessageSquare, Home, FileText } from 'lucide-react';
...
export default function Sidebar({ showServices = true, showNews = true, showQuickLinks = true }: Readonly<SidebarProps>) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const aRes = await fetch('/api/data/articles?summary=1');
  ...
  const quickLinks = [
    { href: '/dich-vu', label: locale === 'en' ? 'Services' : 'Dịch vụ', icon: FileText },
  ...
  const featuredServices = [
    { slug: 'dich-vu-ke-toan-tron-goi', title: 'Kế toán trọn gói', titleEn: 'Full Accounting' },
    { slug: 'bao-cao-thue-thang-quy', title: 'Báo cáo thuế', titleEn: 'Tax Reports' },
    { slug: 'quyet-toan-thue-nam', title: 'Quyết toán thuế', titleEn: 'Tax Settlement' },
    { slug: 'tu-van-thanh-lap-doanh-nghiep', title: 'Thành lập DN', titleEn: 'Business Setup' },
  ];
      ...
      {showServices && (
        <div className="card-elegant p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-display font-semibold text-lg text-ink leading-tight">{locale === 'en' ? 'Featured services' : 'Dịch vụ nổi bật'}</h3>
          </div>
          <div className="space-y-4">
            {featuredServices.map((s) => (
              <Link href={localePath(locale, `/dich-vu/${s.slug}`)} key={s.slug} className="flex gap-4 group items-center">
                 <div className="w-2 h-2 rounded-full bg-secondary opacity-50 group-hover:opacity-100 transition-opacity"></div>
                <div className="flex flex-col justify-center">
                  <h4 className="font-semibold text-ink group-hover:text-primary transition-colors line-clamp-2 text-sm leading-snug">{locale === 'en' ? s.titleEn : s.title}</h4>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div className="card-elegant p-6">
          ...
          <div className="space-y-4">
            {latestNews.map((a: any) => (
              <Link href={localePath(locale, `/kien-thuc/${a.slug}`)} key={a.id} className="flex gap-4 group">
                  <div className="w-16 h-16 bg-sand rounded-xl overflow-hidden shrink-0">
                    <img src={a.thumbnail || '/images/default-article.svg'} alt={a.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
